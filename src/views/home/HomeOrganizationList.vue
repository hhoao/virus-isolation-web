<template>
  ...
  <div class="border-2 border-solid pt-2 rounded-lg h-full">
    <div class="px-4 text-left overflow-y-scroll overflow-x-hidden min-h-36 max-h-41">
      ...
      <template v-if="!listLoading">
        <div v-for="item of organizationList" :key="item.id">
          ...
          >
            ...
            <a class="pl-2 text-gray-800" @click="setCurrentOrganization(item)">{{ item.name }}</a>
          </div>
        </div>
      </template>
    </div>
